...
function IssueCard({ issue, invoiceLines, onSend, onIsolate, onEdit }: IssueCardProps) {
  ...
  // PROJ-43: pending border is amber (same as escalated)
  // PROJ-39: escalated border overrides severity border
  const borderColor = (isPending || isEscalated)
    ? 'border-l-amber-400'
    ...
    <div className={`enterprise-card border-l-4 ${borderColor}`}>
      ...
      <div className="flex items-start justify-between gap-4 px-4 pt-4 pb-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* PROJ-43: Pending status indicator */}
          {isPending && (
            <div className="mt-1 flex items-center gap-1 text-xs text-amber-500">
              <Hourglass className="w-3 h-3" />
              <span>In Klaerung</span>
              {issue.escalatedTo && <span>— Mail an {issue.escalatedTo}</span>}
              {issue.escalatedAt && <span>seit {formatTimestamp(issue.escalatedAt)}</span>}
            </div>
          )}
          {/* PROJ-39: Escalation info line (open but escalated) */}
        ...
        <div className="flex items-center gap-1 flex-shrink-0 pt-0.5">
          ...
          {/* PROJ-43: Bearbeiten/Erneut senden/Anfrage pruefen based on status */}
          {isPending ? (
            <Button
              variant="outline"
              size="sm"
              className="gap-1 text-xs h-7 px-2 border-amber-400 text-amber-600 hover:bg-amber-50/20"
              onClick={() => onEdit ? onEdit(issue) : onSend(issue)}
            >
...
export function IssuesCenter() {
  ...
  // PROJ-43: Aktualisieren-Button spin state
  ...
  // PROJ-43: Pending issues — escalated, awaiting external response
  ...
  const escalatedCount = openIssues.filter(i => !!i.escalatedAt).length + pendingIssues.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="enterprise-card p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <Button
            ...
            disabled={!currentRun}
            title="Issues neu auswerten (keine Steps werden erneut gestartet)"
          >
      ...
      {/* Issues List */}
      <div className="space-y-6">
        {openIssues.length === 0 && pendingIssues.length === 0 && resolvedIssues.length === 0 ? (
          ...
          <>
            ...
            {/* PROJ-43: Pending issues — "In Klaerung" section */}
            {pendingIssues.length > 0 && (
              <div className="mt-6">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-amber-300 mb-2">
                  In Klaerung
                  <span className="ml-2 text-xs font-normal normal-case px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 border border-amber-500/40">
                    {pendingIssues.length} ausstehend
                  </span>
                </h3>
      ...
      {/* PROJ-43: IssueDialog — Tabbed issue management dialog */}
