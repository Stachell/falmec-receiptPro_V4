...
Der bisherige Zähler-Text `{filteredLines.length} von {invoiceLines.length} Positionen` wird vollständig entfernt und durch das Icon ersetzt.
...
| Status | Bedeutung |
|--------|-----------|
| `'ok'` | Alle Artikel haben Bestellungen — vollständig |
...
|---------|------|---------|
| Gesperrt (Step 4 nicht fertig) | Emoji `🔒` | "Gesperrt/locked: Artikelliste wird nach Abschluss von Schritt 4 ausgerollt und ist ab dann verfügbar." |
| Freigegeben (Step 4 fertig) | Emoji `🔓` | "Artikelliste zur Bearbeitung freigegeben" |
...
- **Icon:** Emoji-Zeichen (kein lucide-react Icon), Größe `2.156rem` (= `text-3xl` × 1.15, d.h. 115% von 1.875rem)
  ...
  <div className="flex items-center">
    <div
      ...
      title={isStep4Done
        ...
        : 'Gesperrt/locked: Artikelliste wird nach Abschluss von Schritt 4 ausgerollt und ist ab dann verfügbar.'}
    >
      {isStep4Done ? (
...
- `src/components/run-detail/ItemsTable.tsx` — einzige Code-Änderung

## Dependencies

- Kein neuer State, kein Store-Eingriff
- Nutzt bestehende `currentRun.steps[]` aus `useRunStore()` (bereits im Component vorhanden)
- Keine lucide-react Icons mehr (Lock/Unlock aus Import entfernt)

## ADD-ON Änderungshistorie

| Version | Änderung |
|---------|----------|
| Initial | Lock/Unlock im rechten Separator-Wrapper (w-24) neben Chevron-Button |
| ADD-ON 1 | Position in Filter-Bereich verschoben (ml-5 nach Filter), Chevron-Wrapper wiederhergestellt, Icon-Größe h-9 w-9, Lock `text-foreground` |
| ADD-ON 2 | Icons auf Emoji 🔒/🔓 umgestellt (`text-3xl`), Tooltip-Text angepasst, lucide-react Lock/Unlock Imports entfernt |
| ADD-ON 3 | Position geändert: Icon vor Titel-Block (15px Abstand rechts), aus Filter-Bereich entfernt; Icon-Größe auf 120% (`2.16rem`) erhöht |
| ADD-ON 4 | Icon-Größe auf 115% (`2.156rem`) angepasst, Abstand zum Titel von 15px auf 10px verringert |
