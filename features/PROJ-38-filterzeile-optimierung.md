# PROJ-38: Optimierung, Anpassung und Verlinkung der Filterzeile + Filterzeile RE-Positionen (Rev 5)
...
|-----|-------|-------------|
| 0 | 2026-02-28 | Initialer Plan â€” Aufgabe 1 (Filter-Aktualisierung) + Aufgabe 2 (RE-Positionen Filter) + Stolpersteine |
...
| 4 | 2026-02-28 | **3 Code-Leitplanken:** (1) Explizites UI-Alignment â€” exakte Tailwind-Klassen fuer 1:1 optischen Klon. (2) `<SelectGroup>` statt `<Fragment>` fuer Artikelliste-Gruppen. (3) Null-Safety via Optional Chaining in allen Such-/Filterausdruecken. **Neue Stolperfalle S9:** Body-Abschluss-Schutz â€” collapsedHeight-Berechnung darf nicht brechen. |
| 5 | 2026-03-02 | **ADD-ON Such-Normalisierung integriert:** Offene Suche in Artikelliste + RE-Positionen nutzt zentrale `normalizeSearchTerm`-Funktion (lowercase + trim + Entfernen von Leerzeichen, Punkten, Bindestrichen, Slashes und `#`). Dropdown-Filter bleibt unveraendert. Unit-Tests fuer Normalisierung hinzugefuegt. |
...
### L1: Explizites UI-Alignment â€” 1:1 optischer Klon [NEU in Rev 4]
...
Input:          "pl-10 bg-surface-elevated"          â† KEINE h-8, KEINE text-sm
...
SelectTrigger:  "w-[240px] bg-surface-elevated"       â† erbt h-10 aus shadcn-Default (select.tsx Z.20)
...
```html
<CardHeader className="flex flex-row items-center gap-4 pb-2">          â† fehlt flex-wrap
  <div className="relative flex-1 max-w-xs">                            â† max-w-xs statt max-w-sm
    <Input className="pl-10 h-8 text-sm bg-surface-elevated" />          â† h-8 + text-sm falsch
  </div>
...
**SOLL (nach PROJ-38):**
```html
<CardHeader className="flex flex-row flex-wrap items-center gap-4 pb-2">  â† flex-wrap ergaenzt
  <div className="relative flex-1 max-w-sm">                              â† max-w-sm (wie ItemsTable)
    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
    <Input
      placeholder="Pos., EAN, Artikelnr. suchen..."
      className="pl-10 bg-surface-elevated"                                â† h-8 + text-sm ENTFERNT
    />
  </div>
  <div className="flex items-center gap-2">                                â† NEU: Filter-Container
    <Filter className="w-4 h-4 text-muted-foreground" />
    <Select value={statusFilter} onValueChange={setStatusFilter}>
      <SelectTrigger className="w-[240px] bg-surface-elevated">            â† h-10 erbt aus shadcn
  ...
  <div className="ml-auto flex items-stretch">                             â† Titel bleibt rechts
    ...
  </div>
</CardHeader>
```
...
Im gruppierten Dropdown der Artikelliste (`ItemsTable.tsx`) muss **`<SelectGroup>`** (aus `@/components/ui/select`) als Wrapper um jede Filtergruppe verwendet werden â€” NICHT `<Fragment>`.
...
**Hinweis:** `InvoicePreview.tsx` verwendet eine flache Liste ohne Gruppen â†’ `SelectGroup` wird dort NICHT benoetigt.
...
#### ItemsTable.tsx â€” Suchlogik (Zeile 76-82, aktuell UNSICHER):

```typescript
// IST (UNSICHER â€” crasht bei null):
...
// SOLL (SICHER â€” mit Optional Chaining):
line.manufacturerArticleNo?.toLowerCase().includes(term) ||
...
**Achtung:** `line.ean.includes(searchTerm)` im IST-Zustand verwendet KEIN `.toLowerCase()` â€” das ist ein bestehender Bug (case-sensitive EAN-Suche). Bei der Korrektur auf `?.toLowerCase().includes(term)` umstellen (konsistent mit den anderen Feldern).

#### InvoicePreview.tsx â€” Suchlogik (Zeile 129-137, bereits SICHER):

Die bestehende Suchlogik in InvoicePreview verwendet bereits `?.` â€” hier kein Handlungsbedarf:
```typescript
...
#### filterConfig.ts â€” matchesInvoiceActionFilter:
...
### 1.1 IST-Zustand â€” Aktuelle Filter
...
### 1.2 SOLL-Zustand â€” Aktualisierte & erweiterte Filter
...
+------------------------------+
|  Alle anzeigen               |
| â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ |  <- SelectSeparator
...
|---|-------|-------|-------------|-------|
| 0 | `all` | Alle anzeigen | â€” | Kein Filter |
...
| 4 | `action-conflict` | Konflikte | **Hybrid** (PDF + enriched) | Zusammengesetzter Check â€” siehe 2.4 |
...
### 2.4 Filterlogik Detail â€” Hybrid-Pruefung
...
#### `action-price` â€” "Preisabweichungen"
- **Datenquelle:** `positionStatusMap` â†’ `representativeLine.priceCheckStatus`
- **Logik:** `line.priceCheckStatus === 'mismatch'`
- **Vor Step 2:** `posStatus` nicht vorhanden â†’ `return false` (kein Preis-Check erfolgt, nichts anzeigen)

#### `action-match` â€” "Matchstatus pruefen"
- **Datenquelle:** `positionStatusMap` â†’ `representativeLine.matchStatus`
- **Logik:** `line.matchStatus !== 'full-match'`
- **Vor Step 2:** `posStatus` nicht vorhanden â†’ `return true` (alle Positionen sind implizit "noch nicht gematcht" â†’ anzeigen)

#### `action-order` â€” "Bestellung fehlt"
...
#### `action-conflict` â€” "Konflikte"
- **Datenquelle:** **Hybrid** â€” `pos` (PDF) + `positionStatusMap` (enriched)
- **Logik (Oder-Verknuepfung):**
  1. `!pos.ean?.trim()` â€” EAN fehlt im PDF (rohe PDF-Daten, L3 null-safe)
  2. `line.priceCheckStatus === 'missing'` â€” Kein Sage-Preis vorhanden (enriched)
  3. `line.matchStatus === 'no-match'` â€” Kein Artikelmatch gefunden (enriched)
  4. `!line.activeFlag` â€” Artikel in Sage als inaktiv markiert (enriched)
- **Vor Step 2:** Nur Punkt 1 (EAN-Check, PDF-basiert) wird ausgefuehrt. Punkte 2-4 uebersprungen.

### 2.5 Formatierung â€” 1:1 Klon der Artikelliste (L1)
...
```typescript
/** "Alle anzeigen" â€” wird von BEIDEN Filtern geteilt */
 ...
 *   - `pos`: Rohe PDF-Parse-Daten (ParsedInvoiceLineExtended) â€” immer verfuegbar
 *   - `line`: Enriched Store-Daten (InvoiceLine) â€” erst ab Step 2 verfuegbar (optional)
 *
...
## Zusammenfassung der Dateiaenderungen (Rev 5)

| Datei | Aenderung |
|-------|-----------|
| **`src/lib/filterConfig.ts`** | **NEU** â€” Typen, `FILTER_ALL`, `ITEMS_FILTER_GROUPS[]`, `INVOICE_ACTION_FILTERS[]`, `matchesItemsStatusFilter()`, `matchesInvoiceActionFilter()` |
| `src/components/run-detail/ItemsTable.tsx` | Filterlogik â†’ `matchesItemsStatusFilter()`, Dropdown â†’ `ITEMS_FILTER_GROUPS.map()` mit `<SelectGroup>` (L2), Suchlogik â†’ Null-Safety `?.` (L3), Dropdown-Breite `w-[240px]`, Import `SelectGroup` + `SelectSeparator` + `SelectLabel` |
| `src/components/run-detail/InvoicePreview.tsx` | Filter-Dropdown NEU (flach, `INVOICE_ACTION_FILTERS`), CardHeader â†’ 1:1 Klon (L1), Input `h-8 text-sm` entfernen, `max-w-xs` â†’ `max-w-sm`, `flex-wrap` ergaenzen, `statusFilter` State, Filterlogik via `matchesInvoiceActionFilter()` |
| `src/components/ui/select.tsx` | **KEIN** Aenderungsbedarf â€” `SelectGroup`, `SelectLabel`, `SelectSeparator` sind bereits exportiert (Z.9, 93-99, 124-130, 132-143) |

---

## Stolpersteine

### S1: positionStatusMap-Abhaengigkeit â€” Differenziertes Verhalten pro Action-Filter
...
CardHeader in InvoicePreview erhaelt `flex flex-row flex-wrap items-center gap-4 pb-2` â€” 1:1 identisch zur Artikelliste (L1).

### S9: Body-Abschluss-Schutz â€” collapsedHeight darf nicht brechen [NEU in Rev 4]
...
- â†’ `collapsedHeightPx` wird automatisch kleiner â†’ Tabelle passt sich an
- â†’ Der sticky Toggle-Button am unteren Rand (`sticky bottom-0`) bleibt positioniert
- â†’ Die `useEffect`-Abhaengigkeit auf `filteredPositions.length` triggered ein Re-Measure bei Filterwechsel
...
1. Jeden der 16 Filter einzeln aktivieren â†’ gefilterte Zeilen korrekt?
2. `<SelectGroup>` + `<SelectLabel>` korrekt gerendert? (L2)
3. Dropdown-Breite `w-[240px]` â€” kein Textabschnitt?

### RE-Positionen (Aufgabe 2)
4. "Alle anzeigen" â†’ alle Positionen sichtbar
5. "Preisabweichungen" â†’ nur Positionen mit `priceCheckStatus === 'mismatch'`
6. "Matchstatus pruefen" â†’ alle Positionen mit `matchStatus !== 'full-match'`
7. "Bestellung fehlt" â†’ nur Positionen mit `orderStatus === 'NO'` (PDF-basiert)
8. "Konflikte" â†’ EAN fehlt ODER Preis fehlt ODER no-match ODER inaktiv

### Pre-Step-2-Verhalten (RE-Positionen)
9. "Preisabweichungen" vor Step 2 â†’ Tabelle leer
10. "Matchstatus pruefen" vor Step 2 â†’ ALLE Positionen sichtbar
11. "Bestellung fehlt" vor Step 2 â†’ funktioniert (PDF-only)
12. "Konflikte" vor Step 2 â†’ nur Positionen mit fehlender EAN

### UI-Alignment (L1)
13. Tab-Wechsel Artikelliste â†” RE-Positionen: Suchleiste + Filter optisch identisch?
...
### Null-Safety (L3)
20. Artikelliste Suche mit Position deren `manufacturerArticleNo` null ist â†’ kein Crash?
21. Artikelliste Suche mit Position deren `ean` null ist â†’ kein Crash?

### DRY-Validierung
22. `matchesItemsStatusFilter` und `matchesInvoiceActionFilter` voneinander unabhaengig?
23. Neuen Filter in `filterConfig.ts` hinzufuegen â†’ erscheint nur in der richtigen Komponente?
...
- **ItemsTable:** `itemsFilterCounts: Map<string, number>` â€” Dependency: `[invoiceLines]`
- **InvoicePreview:** `invoiceFilterCounts: Map<string, number>` â€” Dependency: `[positions, positionStatusMap]`
...
**UX-Sonderregel (ItemsTable):** Die Gruppen-Struktur (`<SelectGroup>`, `<SelectLabel>`, `<SelectSeparator>`) bleibt IMMER sichtbar â€” auch wenn alle Optionen innerhalb einer Gruppe 0 Treffer haben. Das signalisiert dem User: "Hier gibt es keine Probleme."

### Fallback: Reset-Guard (`useEffect`)

Wenn ein aktiver Filter (z. B. "Konflikte") durch Datenkorrektur auf 0 Treffer faellt, verschwindet das `<SelectItem>` aus dem DOM. Radix UI kann dann keinen Wert anzeigen, der nicht existiert. Ein `useEffect` prueft nach jedem Render: Ist `statusFilter !== 'all'` UND Count `=== 0`? â†’ Automatischer Reset auf `'all'`.
...
## ADD-ON: Such-Normalisierung fuer offene Suche [NEU in Rev 5]

**Datum:** 2026-03-02

### Ziel

Die offene Suche soll robust gegen Darstellungsunterschiede bei Artikelnummern sein (z. B. Leerzeichen oder Trennzeichen, die durch PDF-Fragmentierung entstehen), ohne den Dropdown-Filter anzufassen.

### Umsetzung

Es wurde eine zentrale Hilfsfunktion eingefuehrt:
- `src/lib/searchNormalization.ts`
- Funktion: `normalizeSearchTerm(value)`
- Verhalten:
  - lowercase
  - trim
  - entfernt fuer den Vergleich: Leerzeichen, `.`, `-`, `/`, `#`

Diese Normalisierung wurde in beiden Suchpfaden angewendet:
- `src/components/run-detail/ItemsTable.tsx`
- `src/components/run-detail/InvoicePreview.tsx`

Der Dropdown-Filter (Status-/Action-Filter) blieb unveraendert.

### Tests

Unit-Tests fuer die Normalisierung wurden ergaenzt:
- `src/lib/searchNormalization.test.ts`

Abgedeckt sind:
- Null/Undefined
- Gross-/Kleinschreibung + Rand-Whitespace
- Trennzeichen-Mix (` `, `.`, `-`, `/`, `#`)

### Ergebnis

Die offene Suche ist jetzt tolerant gegen typische Separator-Unterschiede in Artikelnummern und bleibt gleichzeitig KISS, da keine Parser-Logik geaendert wurde.
